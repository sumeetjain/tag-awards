  ...
  describe "#weight" do
    context "when user has three viewings" do
      it "changes according to viewings.count" do
        user1 = User.new(first_name: "Ryan")
        ...
        viewing_1 = Viewing.new(play_id: first_play.id, user_id: user1.id)
        viewing_2 = Viewing.new(play_id: second_play.id, user_id: user1.id)
        viewing_3 = Viewing.new(play_id: third_play.id, user_id: user1.id)
        nomination = Nomination.new(user_id: user1.id)
